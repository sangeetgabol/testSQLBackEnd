...
exports.listDatabase = (req, res, next) => {
  Database.find({ creator: req.session.userId }, (err, databases) => {
    if (err) {
      return next(err);
    }
...
exports.saveDatabase = (req, res, next) => {
  ...
  const url = req.protocol + "://" + req.get("host") + "/saves/";
  const database = new Database({
    title: req.params.title,
    path: req.file.filename,
    pathNew: url + req.file.filename + ".sqlite",
    ...
    .exec((err, dependantGroup) => {
      ...
      if (dependantGroup) {
        return res
          .status(400)
          .json(
            `Cannot delete this database as the group "${dependantGroup.title}" uses it.`
          );
      }
